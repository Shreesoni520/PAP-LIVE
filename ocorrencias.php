...
date_default_timezone_set('Europe/Lisbon'); // Portugal continental
...
$logoUrl   = 'https://lucky-plum-7yxiihmoh3.edgeone.app/logom.png'; // teu logo direto (NOVO)
$siteName  = 'Reporta Évora';
$publicUrl = 'http://localhost/PAP/index.php?evora_p=inicio'; // quando tiveres domínio, muda para https://teu-dominio/...
$baseUrl   = 'http://localhost/PAP'; // idem acima

// Tipos de intervenção fixos
$intervencoes = ['Poda', 'Corte', 'Outros'];

// público não escolhe estado/tarefa e não validamos aqui
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($errorMsg !== '') {
        // já existe erro de data
    } elseif ($descricao === '' || $latitude === '' || $longitude === '' || $tipo_intervencao === '') {
        ...
        $pasta_uploads = 'C:/xampp/htdocs/PAP/uploads/ocorrencias';
            ...
            if ($imagem['error'] !== UPLOAD_ERR_OK) {
                ...
                if (!in_array($imagem['type'], $tipos_permitidos)) {
                    ...
                    if (!in_array($extensao, ['jpg', 'jpeg', 'png'])) {
                        ...
                        $imagem_nome = 'ocorrencia_' . time() . '_' . $user_id . '.' . $extensao;
            ...
            if ($stmt) {
                ...
                if ($stmt->execute()) {
                    ...
                    $_SESSION['ocorrencia_msg'] = [
                        'type' => 'success',
                        'text' => 'Ocorrência registada com sucesso!'
                    ];

                    $to        = ADMIN_EMAIL;
                    $subject   = 'Nova ocorrência registada no ' . $siteName;
                    $adminLink = $baseUrl . '/admin/';

                    $body  = '<!DOCTYPE html><html><head><meta charset="UTF-8">';
                    $body .= '<title>Nova ocorrência</title></head>';
                    ...
                    $body .= 'Nova ocorrência registada no espaço público</h2>';
                    $body .= '</td></tr>';

                    $body .= '<tr><td style="padding-bottom:10px;">';
                    $body .= '<p style="margin:0;color:#4b5563;font-size:14px;line-height:1.6;">';
                    $body .= 'Foi registada uma nova ocorrência através do site público ';
                    ...
                    try {
                        ...
                        $smsBody =
                            "Nova Ocorrência Espaço Verde (Público)\n\n" .
                            "Descrição: {$shortDesc}\n" .
        ...
        <div class="container h-100 d-flex align-items-center justify-content-center">
            <div class="row w-100 gy-0 align-items-center justify-content-center">
                <div class="container section-title" data-aos="fade-up" style="margin-top: 5%;">
                    <h2>Registo de Ocorrências</h2>
                    <p>Registe situações no espaço público que necessitam de intervenção técnica.</p>
                </div>
            </div>
        </div>
    </section>

    <section id="registo-ocorrencias" class="contact section">
                        ...
                        <div class="info-item d-flex align-items-start mb-3">
                            ...
                            <div>
                                ...
                                <ul>
                                    ...
                                    <li>Use o mapa para obter coordenadas precisas.</li>
                                    <li>Pesquise o nome do local para preencher automaticamente.</li>
                                    <li>Após o registo verá uma mensagem de sucesso ou erro.</li>
                                </ul>
                            </div>
                        </div>

                        <div class="mt-4">
                            <h5>Mapa da Ocorrência</h5>
                            <p class="text-muted mb-1">
                                Use o mapa para escolher o local exato da ocorrência.
                            </p>
                            <ul class="text-muted mb-2" style="padding-left:18px;">
                                <li>Clique no mapa para definir o ponto.</li>
                                <li>Pode arrastar o marcador para ajustar a posição.</li>
                                <li>A latitude, longitude e nome do local são preenchidos automaticamente.</li>
                            </ul>
                            <div id="map"></div>
                        </div>
                    </div>
                ...
                <div class="col-lg-7">
                    <div class="info-wrap">
                        ...
                        <form action="" method="post" class="row g-3" enctype="multipart/form-data">
                            ...
                            <div class="col-12">
                                <label for="place-name-field" class="form-label">Nome do Local</label>
                                ...
                                <small class="text-muted">
                                    Pode pesquisar por nome, rua, código postal, cidade ou país.
                                </small>
                            </div>
