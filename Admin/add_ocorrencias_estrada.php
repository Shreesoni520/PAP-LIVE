...
// Config para emails HTML (mesmo design dos outros)
...
if (isset($_POST['add_ocorrencia'])) {
    ...
    if (!$error) {
        $uploadDir = dirname(__DIR__) . '/uploads/ocorrencias_estrada';
        if (!is_dir($uploadDir)) {
        ...
        if ($imagem && $imagem['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($imagem['type'], $tipos_permitidos)) {
                ...
                if (!in_array($extensao, ['jpg', 'jpeg', 'png'])) {
                    ...
                    $imagem_nome = 'ocorrencia_estrada_' . time() . '_' . $_SESSION['user_id'] . '.' . $extensao;
        ...
        if (!$error) {
            ...
            // coluna estado na BD continua a ser usada, mas recebe o valor da tarefa
            $estado_para_bd = $tarefa;

            $completedAt = null;
            if ($estado_para_bd === 'Concluída') {
            ...
            if ($stmt !== false) {
                ...
                if ($stmt->execute()) {
                    ...
                    $acao    = 'Ocorrência de estrada registada';
                    $detalhe = "Local: " . ($place_name ?: 'Sem nome') .
                               " · Tipo: $tipo_intervencao · Tarefa: $estado_para_bd";
                    ...
                    ");
                    if ($stmtAt) {
                        $stmtAt->bind_param("iss", $user_id, $acao, $detalhe);
                        $stmtAt->execute();
                        $stmtAt->close();
                    }

                    $to        = ADMIN_EMAIL;
                    $subject   = 'Nova ocorrência de estrada registada (Admin) - ' . $siteName;
                    $adminLink = $baseUrl . '/admin/';

                    $body  = '<!DOCTYPE html><html><head><meta charset="UTF-8">';
                    $body .= '<title>Nova ocorrência de estrada (Admin)</title></head>';
                    ...
                    $body .= 'Nova ocorrência de estrada registada pelo administrador</h2>';
                    $body .= '</td></tr>';

                    $body .= '<tr><td style="padding-bottom:10px;">';
                    $body .= '<p style="margin:0;color:#4b5563;font-size:14px;line-height:1.6;">';
                    $body .= 'Uma nova ocorrência de estrada foi registada no painel de administração por ';
                    ...
                    $body .= '<div>' . htmlspecialchars($estado_para_bd, ENT_QUOTES, 'UTF-8') . '</div>';
                    $body .= '</td>';
                    $body .= '</tr>';
                    ...
                    if (@mail($to, $subject, $body, $headers)) {
                        $success = "Ocorrência de estrada registada com sucesso! Email enviado.";
                    } else {
                        $success = "Ocorrência de estrada registada com sucesso, mas ocorreu um erro ao enviar o email.";
                    }

                    // SMS Twilio só com "Tarefa"
                    try {
                        ...
                        $smsBody =
                            "Nova Ocorrência Estrada (Admin)\n" .
                            "\n" .
                            "Descrição: {$shortDesc}\n" .
                            "Tipo: {$tipo_intervencao}\n" .
                            "Tarefa: {$estado_para_bd}";
        ...
        <div class="page-content">
            <div class="edit-container">
                ...
                <div class="page-heading-custom">
                    ...
                    <p>Registe situações que necessitam de intervenção nas estradas e vias públicas.</p>
                </div>
                            ...
                            <div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea
                                        ...
                                        class="form-control"
                                        required
                                        placeholder="Descreva a situação (buracos, sinalização, pavimento, etc.)"
                                    ></textarea>
                                </div>
                                ...
                                <div class="mb-3">
                                    <label class="field-label mb-1">Data da Ocorrência (opcional)</label>
                                    <input
                                        type="date"
