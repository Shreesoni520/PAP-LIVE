...
========================================================= */
function runAutomaticRoadReminders(mysqli $conn): void {
    $sql = "
        SELECT
            id,
            descricao,
    ...
    while ($ocor = $res->fetch_assoc()) {
        ...
        $tituloNotif = "Lembrete de ocorrência de estrada";

        $mensagemNotif = "Lembrete automático: ainda existe uma ocorrência de estrada pendente. ";
        ...
        $stmtNotif = $conn->prepare("
            ...
            VALUES (?, ?, 'ocorrencia_estrada', ?, ?, ?, NULL, 0, NOW())
        ");
...
function getLatestRoadReport(mysqli $conn, int $occId): string {
    $stmt = $conn->prepare("
        SELECT mensagem
        FROM ocorrencia_estrada_relatorios
        WHERE ocorrencia_estrada_id = ?
...
function upsertSingleRoadNotification(
    mysqli $conn,
    int $adminId,
    int $createdByUserId,
    int $occId,
    ...
    $titulo = 'Atualização da ocorrência de estrada';
    ...
    $stmtClean = $conn->prepare("
        ...
        WHERE user_id = ?
          AND origem_id = ?
          AND lida = 0
          AND origem_tipo = 'ocorrencia_estrada'
          AND titulo IN (
              'Ocorrência de estrada atualizada',
              'Ocorrência de estrada concluída',
...
function notifyAdminsRoadSingleUpdate(
    mysqli $conn,
    int $createdByUserId,
    array $ocorrencia,
    string $estadoAtual,
    ...
    if ($latestReport === '') {
        $latestReport = getLatestRoadReport($conn, $occId);
    }
    ...
    $mensagemNotif  = "Atualização da ocorrência de estrada.\n\n";
    ...
    if ($isConclusion) {
        $mensagemNotif .= "\nResultado: ocorrência de estrada marcada como concluída.\n";
    }

    $mensagemNotif .= "\nAtualizado por: {$updatedBy}.\n";
    $mensagemNotif .= "Clique para abrir os detalhes desta ocorrência de estrada.";
    ...
    while ($admin = $adminsRes->fetch_assoc()) {
        ...
        upsertSingleRoadNotification($conn, $adminId, $createdByUserId, $occId, $mensagemNotif);
    }
}
...
runAutomaticRoadReminders($conn);
...
/* =========================================================
   ASSIGN SELECTED ROAD OCCURRENCES + ONE NOTIFICATION
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_selected']) && $isAdmin) {
    ...
    $mensagemFuncionarioGuardar = $mensagemFuncionario !== ''
        ? $mensagemFuncionario
        : 'Deve verificar o local e resolver a ocorrência de estrada atribuída.';
    ...
    if (empty($selectedIds)) {
        $error = "Selecione pelo menos uma ocorrência de estrada.";
                ...
                try {
                    ...
                    foreach ($selectedIds as $occId) {
                        ...
                        $stmtDeleteOldNotif = $conn->prepare("
                            ...
                            WHERE user_id = ?
                              ...
                              AND titulo IN (
                                  ...
                                  'Nova tarefa de ocorrência de estrada',
                                  'Novas tarefas de ocorrência de estrada'
                              )
                        ");
                        ...
                        regista_log(
                            ...
                            $userId,
                            "editar",
                            "intervencao_ocorrencia_estrada",
                            $occId,
                            "Ocorrência de estrada atribuída ao funcionário"
                        );
                    }
                    ...
                    if ($quantidade === 1) {
                        $tituloNotif = "Nova tarefa de ocorrência de estrada";
                        $mensagemNotif = "Foi-lhe atribuída 1 tarefa de ocorrência de estrada.\n\n";
                    } else {
                        $tituloNotif = "Novas tarefas de ocorrência de estrada";
                        $mensagemNotif = "Foram-lhe atribuídas {$quantidade} tarefas de ocorrência de estrada.\n\n";
                    }
                    ...
                    $stmtInsertOneNotif = $conn->prepare("
                        INSERT INTO notificacoes
                        (user_id, created_by_user_id, origem_tipo, origem_id, titulo, mensagem, enviar_em, lida, criada_em)
                        VALUES (?, ?, 'ocorrencia_estrada', ?, ?, ?, ?, 0, NOW())
                    ");
                    ...
                    header("Location: index.php?evora=editarintervencaoocorrenciasestrada&assigned=1");
                    exit();
                } catch (Throwable $e) {
                    $conn->rollback();
                    $error = "Erro ao atribuir ocorrências de estrada: " . $e->getMessage();
                }
            }
        }
    }
...
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_road_report']) && $id > 0) {
    ...
    if ($reportMessage === '') {
        ...
        if (!$ocorReport) {
            $error = "Ocorrência de estrada não encontrada.";
            ...
            try {
                ...
                $stmtSaveReport = $conn->prepare("
                    INSERT INTO ocorrencia_estrada_relatorios
                    (ocorrencia_estrada_id, funcionario_id, mensagem, criado_em)
                    VALUES (?, ?, ?, NOW())
                ");
                ...
                $acao = 'Relatório de ocorrência de estrada enviado';
                ...
                header("Location: index.php?evora=editarintervencaoocorrenciasestrada&id={$id}&saved=1&report_sent=1");
                exit();
            } catch (Throwable $e) {
                $conn->rollback();
...
========================================================= */
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_intervencao_ocorrencia_estrada']) && $id > 0) {
    $stmtCheck = $conn->prepare("SELECT * FROM ocorrencias_estrada WHERE id = ?");
        ...
        if (!$canEdit) {
            ...
            if ($tarefa === '') {
                ...
                if (!$stmt) {
                    ...
                    regista_log($conn, $userId, "editar", "intervencao_ocorrencia_estrada", $id, "Tarefa: $tarefa");

                    $acao = 'Intervenção de ocorrência de estrada atualizada';
                    ...
                    header("Location: index.php?evora=editarintervencaoocorrenciasestrada&id={$id}&saved=1{$extraReport}");
                    exit();
                } else {
...
/* =========================================================
   PARTE 1 – LISTAR OCORRÊNCIAS ESTRADA PARA INTERVENÇÃO
========================================================= */
if ($id <= 0) {
    $whereSql = '';
    if (!$isAdmin) {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Intervenção - Ocorrências Estrada</title>
        ...
        @media (max-width: 768px) {
            .filter-panel { position: fixed; top: 88px; right: 12px; width: 260px; max-width: 80%; padding: 14px 12px 10px 12px; border-radius: 14px; box-shadow: 0 10px 28px rgba(15, 23, 42, 0.18); }
            .filter-panel .form-label { font-size: 0.75rem; }
            .filter-panel .form-control, .filter-panel .form-select, .ts-wrapper.single .ts-control, .ts-wrapper.single.input-active .ts-control { font-size: 0.8rem; padding: 0.25rem 0.6rem; }
            .filter-panel-actions { gap: 0.35rem; }
            .filter-panel-actions .btn { font-size: 0.75rem; padding: 0.3rem 0.5rem; border-radius: 999px; }
            .card-header-flex { flex-direction: column; align-items: flex-start; gap: 0.6rem; }
            .ocor-card-footer { flex-direction: column; align-items: flex-start; }
            .ocor-top-row { flex-direction: column; align-items: flex-start; }
            .status-chip { align-self: flex-start; }
        }
        @media (max-width: 992px) {
            #main { margin-left: 0 !important; }
            .sidebar-wrapper { position: fixed; top: 0; left: 0; height: 100vh; z-index: 1500; }
    ...
    <div class="page-heading-custom mb-3">
        <h3 class="mb-1"><?= $isAdmin ? 'Gerir intervenções de ocorrências de estrada' : 'As minhas intervenções de ocorrências de estrada' ?></h3>
        <p class="text-subtitle text-muted mb-0">
            <?= $isAdmin ? 'Selecione uma ou várias ocorrências de estrada e atribua ao funcionário.' : 'Veja apenas as ocorrências de estrada atribuídas a si.' ?>
        </p>
    </div>
    ...
    <section class="section">
        <div class="card card-main position-relative">
            <div class="card-header">
                <div class="card-header-flex">
                    <div>
                        ...
                        <span class="section-subtitle">Use os filtros para encontrar rapidamente tipos ou tarefas específicas.</span>
                    </div>
                    <div class="d-flex align-items-center gap-2 flex-wrap justify-content-end top-actions">
                        <?php if ($isAdmin): ?>
                            <button id="toggleSelectMode" class="btn btn-outline-secondary d-flex align-items-center top-action-btn" type="button"><i class="bi bi-check2-square me-1"></i><span>Modo seleção</span></button>
                        ...
                        <?php endif; ?>
                        <button id="openFilterPanel" class="btn btn-outline-secondary d-flex align-items-center top-action-btn" type="button"><i class="bi bi-funnel-fill me-1"></i><span>Filtrar</span></button>
                    </div>
                </div>
            </div>

            <?php if (!empty($_GET['assigned'])): ?><div class="px-3 pt-3"><div class="alert alert-success mb-0">Ocorrências de estrada atribuídas, notificação única criada e lembrete automático ativado.</div></div><?php endif; ?>
            ...
            <div id="filterPanel" class="filter-panel d-none">
                <form id="ocorFilterForm">
                    <div class="mb-2">
                        ...
                        <select id="filterTipo" class="form-select js-nice-select"><option value="">Todos</option><?php if ($tipos_result && $tipos_result->num_rows > 0): ?><?php while ($row = $tipos_result->fetch_assoc()): ?><?php if (!empty($row['tipo_intervencao'])): $val = strtolower($row['tipo_intervencao']); ?><option value="<?= htmlspecialchars($val) ?>"><?= htmlspecialchars($row['tipo_intervencao']) ?></option><?php endif; ?><?php endwhile; ?><?php endif; ?></select>
                    </div>
                    <div class="mb-2">
                        <label for="filterTarefa" class="form-label">Tarefa</label>
                        <select id="filterTarefa" class="form-select js-nice-select"><option value="">Todas</option><?php if ($estados_result && $estados_result->num_rows > 0): ?><?php while ($row = $estados_result->fetch_assoc()): ?><?php if (!empty($row['estado'])): $val = strtolower($row['estado']); ?><option value="<?= htmlspecialchars($val) ?>"><?= htmlspecialchars($row['estado']) ?></option><?php endif; ?><?php endwhile; ?><?php endif; ?></select>
                    </div>
                    <div class="mb-2">
                        <label class="form-label">Estado rápido</label>
                        <div class="d-flex gap-2 flex-wrap" id="statusQuickButtons"><button type="button" class="btn btn-sm btn-outline-secondary quick-status-btn active" data-status="">Todos</button><button type="button" class="btn btn-sm btn-outline-success quick-status-btn" data-status="feito">Feitos</button><button type="button" class="btn btn-sm btn-outline-warning quick-status-btn" data-status="pendente">Pendentes</button></div>
                        <input type="hidden" id="filterStatus" value="">
                    </div>
                    <div class="mb-2"><label for="filterData" class="form-label">Data da ocorrência</label><input type="date" class="form-control" id="filterData"></div>
                    <div class="filter-panel-actions"><button type="button" class="btn btn-light btn-sm" id="clearFilters">Limpar</button><button type="button" class="btn btn-outline-secondary btn-sm" id="closeFilterPanel">Cancelar</button><button type="submit" class="btn btn-primary btn-sm">Aplicar</button></div>
                </form>
            </div>
            ...
            <div class="card-body">
                    ...
                    <?php while ($ocor = $ocorrencias->fetch_assoc()): ?>
                        <?php
                            ...
                            if (!empty($ocor['imagem'])) {
                                ...
                                $caminhoFisico = $_SERVER['DOCUMENT_ROOT'] . '/PAP/uploads/ocorrencias_estrada/' . $nomeImagem;
                                if (file_exists($caminhoFisico)) {
                                    $imgs[] = '/PAP/uploads/ocorrencias_estrada/' . $nomeImagem;
                                }
                            }
                        ...
                        <div class="col ocor-card-container">
                            <div class="ocor-card">
                                <div class="ocor-card-body">
                                    <div class="ocor-top-row">
                                        <div class="ocor-line ocor-description-wrap"><span class="ocor-label">Descrição:</span> <span class="ocor-value ocor-descricao-search"><?= htmlspecialchars($ocor['descricao']) ?></span></div>
                                        <?php if ($isDone): ?><span class="status-chip status-done ocor-status-search">Feito</span><?php else: ?><span class="status-chip status-pending ocor-status-search">Pendente</span><?php endif; ?>
                                    </div>
                                    <div class="ocor-line"><span class="ocor-label">Local:</span> <span class="ocor-value ocor-local-search"><?= $ocor['place_name'] ? htmlspecialchars($ocor['place_name']) : 'Sem nome' ?></span></div>
                                    <div class="ocor-line"><span class="ocor-label">Latitude/Longitude:</span> <span class="ocor-value"><?= htmlspecialchars($ocor['latitude']) ?>, <?= htmlspecialchars($ocor['longitude']) ?></span></div>
                                    <div class="ocor-line"><span class="ocor-label">Tipo de Intervenção:</span> <span class="ocor-value ocor-tipo-search"><?= htmlspecialchars($ocor['tipo_intervencao'] ?: 'Nenhuma') ?></span></div>
                                    <div class="ocor-line"><span class="ocor-label">Tarefa:</span> <span class="ocor-value ocor-tarefa-search"><?= htmlspecialchars($ocor['estado']) ?></span></div>
                                    <?php if ($isDone && $completedByName !== ''): ?><div class="ocor-line"><span class="ocor-label">Concluído por:</span> <span class="ocor-value"><?= htmlspecialchars($completedByName) ?><?= (isset($ocor['completed_by_is_admin']) && (int)$ocor['completed_by_is_admin'] === 1) ? ' (Administrador)' : ' (Funcionário)' ?></span></div><?php endif; ?>
                                    <div class="ocor-line"><span class="ocor-label">Data da ocorrência:</span> <span class="ocor-value ocor-data-search"><?= $textoDataOcorrencia ?></span></div>
                                    <?php if (!empty($assignedName)): ?><div class="ocor-line"><span class="ocor-label">Funcionário:</span> <span class="ocor-value"><?= htmlspecialchars($assignedName) ?></span></div><?php else: ?><div class="ocor-line"><span class="ocor-label">Funcionário:</span> <span class="ocor-value text-muted">Ainda não atribuído</span></div><?php endif; ?>
                                    <?php if (!empty($scheduledText)): ?><div class="ocor-line"><span class="ocor-label">Data/Hora:</span> <span class="ocor-value"><?= htmlspecialchars($scheduledText) ?></span></div><?php endif; ?>
                                    <?php if (!empty($imgs)): ?><div class="ocor-line ocor-image-line"><span class="ocor-label">Imagem da ocorrência:</span><button type="button" class="btn btn-outline-secondary ocor-image-btn d-inline-flex align-items-center js-open-gallery" data-images='<?= $imgsJson ?>' data-start-index="0"><i class="bi bi-card-image me-1"></i><span>Ver imagem</span></button></div><?php endif; ?>
                                </div>
                                <div class="ocor-card-footer" data-data-ocorrencia="<?= htmlspecialchars($dataOcorrenciaAttr) ?>">
                                    ...
                                    <div class="footer-actions-inline">
                                        <form method="post" action="index.php?evora=editarintervencaoocorrenciasestrada" class="d-inline"><input type="hidden" name="id" value="<?= (int)$ocor['id'] ?>"><button type="submit" class="btn btn-sm btn-primary">Editar</button></form>
                                        <?php if ($isAdmin): ?><div class="form-check mb-0 multi-checkbox-wrapper"><input class="form-check-input ocor-checkbox" type="checkbox" value="<?= (int)$ocor['id'] ?>" id="ocorChk<?= (int)$ocor['id'] ?>"><label class="form-check-label" for="ocorChk<?= (int)$ocor['id'] ?>">Selecionar</label></div><?php endif; ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <?php else: ?>
                    <div class="alert alert-info text-center mb-0"><?= $isAdmin ? 'Nenhuma ocorrência de estrada registada.' : 'Não tem ocorrências de estrada atribuídas.' ?></div>
                <?php endif; ?>
            </div>
        </div>
    </section>
    ...
    <div class="modal fade" id="assignModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered"><div class="modal-content" style="border-radius:16px; border:0; box-shadow:0 18px 45px rgba(15,23,42,0.18);"><form method="post" id="assignForm">
            <div class="modal-header border-0 pb-2"><h5 class="modal-title">Enviar intervenção</h5><button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Fechar"></button></div>
            <div class="modal-body pt-2">
                <div class="mb-3"><label class="form-label fw-semibold">Funcionário</label><select name="assigned_to_user_id" class="form-select js-nice-select" required><option value="">Selecione</option><?php foreach ($workers as $worker): ?><option value="<?= (int)$worker['id'] ?>"><?= htmlspecialchars($worker['display_name']) ?></option><?php endforeach; ?></select></div>
                <div class="alert alert-primary py-2 mb-3" style="font-size: 0.85rem;"><strong>1. Data/Hora para realizar o trabalho</strong><br>Esta data indica quando o funcionário deve ir ao local para resolver a ocorrência de estrada.</div>
                ...
                <div class="alert alert-warning py-2 mb-3" style="font-size: 0.85rem;"><strong>2. Data/Hora para enviar a notificação</strong><br>Se deixar vazio, a notificação aparece imediatamente ao funcionário.</div>
                ...
                <div class="alert alert-info py-2 mb-3" style="font-size: 0.85rem;"><strong>Lembrete automático:</strong><br>Depois da primeira notificação, o sistema irá enviar um novo lembrete a cada 5 dias, até a intervenção ser marcada como concluída.</div>
                <div class="mb-3"><label class="form-label fw-semibold">Mensagem para o funcionário</label><textarea name="mensagem_funcionario" class="form-control" rows="3" placeholder="Ex: Verificar o local, reparar o pavimento, avaliar sinalização ou resolver a ocorrência."></textarea><small class="text-muted">Esta mensagem será enviada com a notificação e ficará visível nos detalhes.</small></div>
                ...
                <div class="small text-muted mb-2" id="selectedSummaryModal">0 cartões selecionados.</div><div id="selectedHiddenInputs"></div>
                <div class="small text-muted">Se deixar data/hora vazias, a intervenção fica disponível imediatamente.</div>
            </div>
...
/* =========================================================
   PARTE 2 – EDITAR INTERVENÇÃO DA OCORRÊNCIA ESTRADA
========================================================= */
$stmt = $conn->prepare("
    SELECT o.*,
           u1.username AS assigned_to_username,
...
if (!$ocorrencia) {
    ?>
    <!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8" /><title>Ocorrência de estrada não encontrada</title><link rel="stylesheet" href="assets/css/bootstrap.css" /><link rel="stylesheet" href="assets/css/app.css" /></head><body><div class="container mt-4"><div class="alert alert-danger mt-5 mx-auto text-center" style="max-width:500px;">Ocorrência de estrada não encontrada.</div></div></body></html>
    <?php
    exit();
}
...
if (!$canView) {
    ?>
    <!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8" /><title>Acesso negado</title><link rel="stylesheet" href="assets/css/bootstrap.css" /><link rel="stylesheet" href="assets/css/app.css" /></head><body><div class="container mt-4"><div class="alert alert-warning mt-5 mx-auto text-center" style="max-width:540px;">Não tem permissão para abrir esta ocorrência de estrada.</div></div></body></html>
    <?php
    exit();
}
...
$stmtRelatorios = $conn->prepare("
    ...
    FROM ocorrencia_estrada_relatorios r
    LEFT JOIN users u ON u.id = r.funcionario_id
    WHERE r.ocorrencia_estrada_id = ?
...
if (!empty($ocorrencia['imagem'])) {
    ...
    $caminhoFisico = $_SERVER['DOCUMENT_ROOT'] . '/PAP/uploads/ocorrencias_estrada/' . $nomeImagem;
    if (file_exists($caminhoFisico)) {
        $imagemUrl = '/PAP/uploads/ocorrencias_estrada/' . $nomeImagem;
    }
}
...
<head>
    <meta charset="UTF-8">
    <title>Editar Intervenção da Ocorrência de Estrada</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
        ...
        <div class="page-content">
            <div class="edit-container">
                <div class="page-heading-custom d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <div><h3>Ocorrência de Estrada</h3><p>Atualize a tarefa da intervenção da ocorrência de estrada selecionada.</p></div>
                    <span class="worker-chip <?= htmlspecialchars($statusClass) ?>"><i class="bi <?= htmlspecialchars($statusIcon) ?>"></i><?= htmlspecialchars($statusLabel) ?></span>
                </div>
                <div class="edit-card">
                    ...
                    <?php if ($mensagemFuncionarioDetalhe !== ''): ?>
                        <div class="admin-message-box"><div class="admin-message-box-title"><i class="bi bi-chat-left-text-fill"></i>Mensagem do administrador</div><div class="admin-message-box-text"><?= nl2br(htmlspecialchars($mensagemFuncionarioDetalhe)) ?></div></div>
                    <?php endif; ?>

                    <?php if ($isDone && $completedByName !== ''): ?>
                        <div class="mb-3"><span class="badge bg-light text-dark border">Concluído por: <?= htmlspecialchars($completedByName) ?><?= $completedByIsAdmin ? ' (Administrador)' : ' (Funcionário)' ?></span></div>
                    <?php endif; ?>

                    <form method="post" autocomplete="off" id="editRoadForm">
                        ...
                        <div class="edit-two-cols mb-3">
                            <div>
                                ...
                                <div class="row">
                                    <div class="col-md-6"><div class="mb-3"><label class="field-label mb-1">Tipo de Intervenção</label><select id="tipo_intervencao" class="form-select js-nice-select" disabled><option value="">Selecione uma opção</option><?php foreach ($intervencoes as $interv) { ?><option value="<?= htmlspecialchars($interv) ?>" <?= ($ocorrencia['tipo_intervencao'] == $interv ? 'selected' : '') ?>><?= htmlspecialchars($interv) ?></option><?php } ?><?php if (!in_array($ocorrencia['tipo_intervencao'], $intervencoes, true) && !empty($ocorrencia['tipo_intervencao'])): ?><option value="<?= htmlspecialchars($ocorrencia['tipo_intervencao']) ?>" selected><?= htmlspecialchars($ocorrencia['tipo_intervencao']) ?></option><?php endif; ?></select></div></div>
                                    <div class="col-md-6"><div class="mb-3"><label class="field-label mb-1">Tarefa</label><select name="estado" id="estado_select_visible" class="form-select js-nice-select" required><option value="">Selecione uma opção</option><?php foreach ($tarefas as $tarefa_opt) { ?><option value="<?= htmlspecialchars($tarefa_opt) ?>" <?= ($ocorrencia['estado'] == $tarefa_opt ? 'selected' : '') ?>><?= htmlspecialchars($tarefa_opt) ?></option><?php } ?></select></div></div>
                                </div>
                                ...
                                <div class="mb-3"><div class="upload-box"><label class="field-label mb-1">Imagem registada</label><?php if ($imagemUrl !== ''): ?><div class="image-preview-box"><img src="<?= htmlspecialchars($imagemUrl) ?>" alt="Imagem da ocorrência de estrada"></div><?php else: ?><div class="upload-hint">Sem imagem associada.</div><?php endif; ?></div></div>
                                <div class="actions-row-desktop"><button type="submit" name="edit_intervencao_ocorrencia_estrada" class="btn btn-primary btn-main w-100">Guardar Alterações</button></div>
                            </div>
                            <div>
                                <div class="section-label">Localização no mapa</div>
                                <div class="mb-3"><label class="field-label mb-1">Nome do Local</label><div class="input-group"><input type="text" id="place_name" class="form-control readonly-input" value="<?= htmlspecialchars($ocorrencia['place_name']) ?>" readonly><button type="button" class="btn btn-outline-secondary btn-search-round" id="search-location" title="Centrar localização"><i class="bi bi-search"></i></button></div></div>
                                <div class="row mb-2"><div class="col-6"><label class="field-label mb-1">Latitude</label><input type="text" id="latitude" class="form-control readonly-input" value="<?= htmlspecialchars($ocorrencia['latitude']) ?>" readonly></div><div class="col-6"><label class="field-label mb-1">Longitude</label><input type="text" id="longitude" class="form-control readonly-input" value="<?= htmlspecialchars($ocorrencia['longitude']) ?>" readonly></div></div>
                                <div class="edit-map-wrapper"><div class="edit-map-top"><span class="edit-map-info">Visualização da localização atual da ocorrência de estrada.</span></div><div id="map"></div></div>
                            </div>
                        </div>
                    </form>
                    ...
                    <div class="reports-section">
                        <div class="reports-header-box"><div class="reports-header-title"><div class="reports-header-icon"><i class="bi bi-chat-square-text-fill"></i></div><div><h5>Relatórios do funcionário</h5><small>Mensagens enviadas pelo funcionário sobre esta intervenção.</small></div></div><div class="reports-count-badge"><?= count($relatorios) ?> relatório(s)</div></div>
                        <?php if (!empty($relatorios)): ?>
                            <div class="report-grid">
                            <?php foreach ($relatorios as $relatorio): ?>
                                <div class="report-card"><div class="report-top"><div class="report-user"><div class="report-avatar"><i class="bi bi-person-circle"></i></div><div><div class="report-author"><?= htmlspecialchars($relatorio['funcionario_nome'] ?? 'Funcionário') ?></div><div class="report-subtitle">Funcionário responsável</div></div></div><div class="report-date"><i class="bi bi-calendar3 me-1"></i><?= !empty($relatorio['criado_em']) ? date('d/m/Y H:i', strtotime($relatorio['criado_em'])) : '—' ?></div></div><div class="report-message-box"><div class="report-message-label"><i class="bi bi-pencil-square"></i>Situação encontrada</div><div class="report-message"><?= nl2br(htmlspecialchars(trim((string)$relatorio['mensagem']))) ?></div></div></div>
                            <?php endforeach; ?>
                            </div>
                        <?php else: ?>
                            <div class="empty-report-box"><i class="bi bi-inbox"></i><div><strong>Ainda não existem relatórios.</strong><br><span>Quando o funcionário enviar uma mensagem, ela irá aparecer aqui.</span></div></div>
                        <?php endif; ?>
                    </div>

                    <?php if (!$isAdmin && (int)$ocorrencia['assigned_to_user_id'] === $userId): ?>
                        <hr class="my-4">
                        <form method="post" autocomplete="off" id="sendRoadReportForm">
                            <input type="hidden" name="id" value="<?= (int)$id ?>">
                            <input type="hidden" name="estado_from_report_form" id="estado_hidden_for_report" value="">
                            <div class="section-label">Mensagem para o administrador</div>
                            <div class="mb-3"><label class="field-label mb-1">Relatório / Situação encontrada</label><textarea name="report_message" id="report_message_visible" class="form-control" rows="4" maxlength="1500" required placeholder="Ex: O pavimento está danificado, a sinalização precisa de reparação, existe perigo no local, ou a ocorrência já foi resolvida."></textarea><small class="text-muted">Escreva aqui o que encontrou no local. O administrador irá receber esta mensagem por notificação.</small></div>
                            <button type="submit" name="send_road_report" class="btn btn-warning w-100"><i class="bi bi-chat-dots me-1"></i> Enviar relatório ao administrador</button>
                        </form>
                    <?php endif; ?>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const editRoadForm = document.getElementById('editRoadForm');
    const sendRoadReportForm = document.getElementById('sendRoadReportForm');
    ...
    editRoadForm?.addEventListener('submit', function () {
        syncReportToSaveForm();
    });

    sendRoadReportForm?.addEventListener('submit', function () {
